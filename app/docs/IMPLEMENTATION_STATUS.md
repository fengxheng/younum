...
* **分类的图片图标**：数据层已完成（`category_icon_asset` 表 + v3→v4 迁移、
  图片校验规则、存储读写、真机验证），**选图与界面接线还没做** ——
  界面上「从图片选择」仍然如实提示这一点（见 `DECISIONS.md` 第 51 节）。
...
| 项目 | 命令 | 结果 |
| --- | --- | --- |
| 单元测试（包含界面测试） | `flutter test` | **400 passed** |
| 真机数据库测试 | `flutter test integration_test/database_test.dart -d 412913d4` | **39 passed** |
...
* **支付宝导出未验证**：微信的 XLSX 已经跑通，支付宝的列名与格式还没见过真文件。
* **「用户真的在系统选择器里点一份账单」未验证** —— 那一步需要人手点，
  自动化点不可靠。清单在 `docs/MANUAL_CHECKS.md` 第一节。
  这是阶段 3 剩下最大的不确定性。
* 平台适配器的完整字段集仍是「通用表头别名匹配」，没有按平台写专用适配器。
...
2. **阶段 4 剩余**：分类图片图标的**选图与界面接线**（系统选图、居中方形
   裁切缩略图、编辑器预览与保存、资源清理）、分类归档入口（原型没有这个
   入口，需要先确认要不要做）。
