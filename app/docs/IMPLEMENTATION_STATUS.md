...
* **退款解除关联**（指南 3.5.7）：建立关联已经能用了，但已经关联上的退款
  没有「取消关联」的入口，也不能撤销那一次关联（阶段 4 剩余）。
...
| 项目 | 命令 | 结果 |
| --- | --- | --- |
| 单元测试（包含界面测试） | `flutter test` | **364 passed** |
| 真机数据库测试 | `flutter test integration_test/database_test.dart -d 412913d4` | **33 passed** |
...
2. **阶段 4 剩余**：分类管理（新建 / 改名 / 图标落盘）、退款解除关联
   （指南 3.5.7）、拆分消费的退款分配（指南 3.5.5）。
