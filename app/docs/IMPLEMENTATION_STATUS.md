...
**分类管理（新建 / 改名 / 图标）仍写内存，导出（PNG / CSV）仍未接入文件写入。**
...
| 项目 | 命令 | 结果 |
| --- | --- | --- |
| 单元测试（包含界面测试） | `flutter test` | **421 passed** |
...
* **银行卡**：`looksLikeXlsx` 按**内容**认出它其实是 XLSX，78 行全部读出，
  但表头（`交易时间 / 付款方姓名 / … / 交易类型 / 交易金额 / … / 交易流水号`）
  匹配不上别名表，于是如实返回 `ImportMappingRequired` 进人工映射页 ——
  没有崩，也没有按固定列下标瞎猜。这是设计要的行为，不是缺陷。
...
* **「用户真的在系统选择器里点一份账单」未验证** —— 那一步需要人手点，
  ...
  这是阶段 3 剩下最大的不确定性。
* 平台适配器仍是「通用表头别名匹配」，没有按平台写专用适配器。
  银行卡那份表头匹配不上别名表，要进人工映射页。⚠️ 但**不能**图省事把
  「交易类型」直接加进方向字段的别名表：微信账单里也有一列叫「交易类型」
  （值是「商户消费」这种），而它排在「收/支」列前面 —— 一加，方向列就会被
  它抢走，微信账单的方向全判错。银行适配器要按**整套表头**识别（像平台名
  识别那样），而不是往公共别名表里塞词。
...
* 微信、支付宝、银行卡三种真实导出都已验过（见上）。**仍未验证**的是：
  银行卡那份在映射页上「用户手工指列 → 提交」的界面路径（解析与提交已验，
  界面要靠人手点），以及「跨来源疑似重复」在真机上逐组确认的交互。
