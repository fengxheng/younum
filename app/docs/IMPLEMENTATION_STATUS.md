...
**阶段 0（设计核对与决策记录）+ 阶段 1（工程与设计系统）+ 阶段 2
（数据底座与账目规则）已完成。**

界面按指南第 5 节的 33 个设计状态全部实现并可真机运行。
阶段 2 起账单已经落在本地 SQLite；**首页、整理页、月报总览与分类占比**
的数字来自真实查询，月报/趋势/明细/分享/我的的一部分仍是设计稿样例
（阶段 5 接完），详见「未完成及风险」。
...
* **月报、趋势、明细、分享、我的**里的数字仍是 `SampleData` 样例。
  首页与整理页已经是真实查询，但点进明细 / 趋势会看到样例数字，
  两边不一致 —— 这是阶段 5 的任务，不是已完成项。
* 拆分、非消费（收入 / 转账 / 排除统计）、退款关联的**界面**未接真实数据；
  规则函数与仓库入口已实现并有测试覆盖，但用户点不到（阶段 4）。
* 分类管理（新建 / 改名 / 图标）仍写内存，重启回到出厂图标（阶段 4）。
* 导入流程未接数据库（阶段 3）；`import_batch` / `import_row` /
  `transaction_origin` / `category_icon_asset` 四张表尚未建。
...
* **首页 / 整理页已是真实查询；月报、趋势、明细、分享、我的仍是样例。**
  阶段 5 必须把剩下的接完，并保证同一笔交易在所有页面金额一致。
...
1. **阶段 5 收尾（把剩下的样例换掉）**：月报 / 趋势 / 明细 / 分享接真实的
   `MonthlyStats` 与 `MonthOverview` —— 它们已经是就绪的纯函数，
   主要工作是改页面取数，不是重写逻辑。趋势需要「有记录的月份」逐月计算，
   不足两个完整月时不显示环比。
2. **阶段 4 界面接线**：详情页改用真实交易 ID、拆分走 `AllocationRules`、
   非消费走交易性质、退款关联走 `RefundRules` + `linkRefund`；
   分类管理落库并补 v1 → v2 迁移。
